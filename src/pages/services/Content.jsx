            ...
            <Stack spacing={"xl"} mt="lg">
                ...
                {data?.specialization.map((obj, ind) => {
                    return (
                        <>
                            {ind % 2 === 0 ? (
                                <Flex gap={isMobile ? "md" : "100px"} align={"center"} className={classes.top1} wrap={isMobile ? "wrap" : "nowrap"}>
                                    <Stack>
                                ...
                                <Flex gap={isMobile ? "md" : "100px"} align={"center"} className={classes.top1} wrap={isMobile ? "wrap" : "nowrap"} direction={isMobile ? "column-reverse" : ""}>
                                    <img src={obj?.img} width={isMobile ? "80%" : "30%"} />
                                    <Stack>
                                        <Title color={theme.colors.blue} fw={400} order={3}>
                                            {obj?.title}
            ...
            {data?.bottomCards && (
                <Box className={classes.services}>
                    ...
                    <Flex gap="xl" mt="xl" justify={"center"} wrap={"wrap"}>
                        <ServiceCard title="Marker based Augmentation" description="User can see life-like digital overlays and interact with them by pointing a camera at pacific trigger point." />
                        <ServiceCard title="Location based Augmentation" description="Navigate the customer right to your store with using GPS, Compass and Beacons." />
                        <ServiceCard title="Markerless Augmentation" description="Marker less AR is based on the simultaneous localization and mapping (SMAL) technology." />
                    </Flex>
                </Box>
            )}
        </Box>
