...
import { BrandFacebook, BrandInstagram, BrandLinkedin, DeviceMobile } from "tabler-icons-react";
...
const Header = ({ opened, toggle }) => {
    ...
    useEffect(() => {
        axios.get(backendUrl + "/about-us/get_all").then((res) => setData(res.data.data));
    }, []);
    const socialLinks = data[0] || {};
        ...
        <>
            <Box
                ...
                <Flex gap={"lg"} align={"center"}>
                    ...
                    <Flex gap="lg" align="center">
                        <Text>Get Social with us!</Text>
                        {facebook && <BrandFacebook className={classes.icon} onClick={() => window.open(facebook, "_blank")} />}
                        {instagram && <BrandInstagram className={classes.icon} onClick={() => window.open(instagram, "_blank")} />}
                        {linkedIn && <BrandLinkedin className={classes.icon} onClick={() => window.open(linkedIn, "_blank")} />}
                    </Flex>
                </Flex>
            </Box>
            <Box
                ...
                <Flex align={"center"} className={classes.logo} onClick={() => navigate("/")}>
                    <img src={logo} width={170} />
                </Flex>
                <Flex gap={"lg"} align={"center"} className={classes.navigationBar}>
                    <Link className={classes.link} to="/" onClick={() => isMobile && toggle()}>
                        Home
                    </Link>
                    <Menu trigger="hover">
                        <Menu.Target>
                  ...
                  >
                    <Link className={classes.link}>{item.name}</Link>
                            ...
                            <Menu.Item
                                ...
                                <Link className={classes.link} to="/services/web-app-development">
                                    Web App Development
                                </Link>
                            </Menu.Item>
                            <Menu.Item
                                ...
                                <Link className={classes.link} to="/services/mobile-app-development">
                ...
                >
                  ERP & Bussiness Solutions
                            ...
                            <Menu.Item
                                ...
                                <Link className={classes.link} to="/services/digital-marketing-solutions">
                    ...
                    <Link className={classes.link} to="/portfolio" onClick={() => isMobile && toggle()}>
                        Our Projects
                    </Link>
                    <Link className={classes.link} to="/products" onClick={() => isMobile && toggle()}>
                        Our Products
                    </Link>
                    {/* <Link
            className={classes.link}
          ...
          >
            Careers
                    ...
                    <Link className={classes.link} to="/about-us" onClick={() => isMobile && toggle()}>
                        About Us
                    </Link>
                    <Link className={classes.link} to="/blogs" onClick={() => isMobile && toggle()}>
                        Blogs
                    </Link>
                    <Link className={classes.link} to="/contact-us" onClick={() => isMobile && toggle()}>
                        Contact Us
                    </Link>
                </Flex>
